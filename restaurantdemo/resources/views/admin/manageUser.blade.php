    ...
    <table id="users" class="table table-hover table-condensed table-striped table-bordered" cellspacing="0" width="100%">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{$usr->name}}</td>
                <td>{{$usr->email}}</td>
                <td>{{$usr->user_type_name}}</td>
                <td><a data-toggle="modal" data-target="#changepassword" class="btn btn-danger" data-whatever="{{$usr->id}}" class="btn btn-danger">Change Password</a></td>
                <td>
                    @if($usr->status == 1)
                ...
                <td>
                    @if($usr->status == 1)
                        <a data-toggle="modal" data-target="#blockuser"  data-whatever="{{$usr->id}}" class="btn btn-danger">Block</a>
                    @else
                        <a data-toggle="modal" data-target="#reactiveuser" data-whatever="{{$usr->id}}" class="btn btn-primary">Re-Activate</a>
                    @endif
                </td>
            </tr>
            @endforeach
...
<div class="modal fade" id="changepassword" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form method="post" action="{{url('/userChangePassword')}}">
                {{csrf_field()}}
                <div class="modal-header">
...
<div class="modal fade" id="blockuser" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form method="post" action="{{url('/manageUser/delete')}}">
                {{csrf_field()}}
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="exampleModalLabel">Delete</h4>
...
<div class="modal fade" id="reactiveuser" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form method="post" action="{{url('/manageUser/reactive')}}">
                {{csrf_field()}}
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="exampleModalLabel">Delete</h4>
